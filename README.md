...
(`results/opmap/operational_map.png`): **緑=光学** (見失い率 ≤ しきい値 かつ目標精度)、
...
出力は表 (コンソール) + `results/spec/design_spec.png` + `results/spec/run_spec.{json,csv}`。
...
深さ・濁りは `config.toml [deepwater]` で編集。出力は `results/deepwater/` (図 + JSON/CSV + README)。
